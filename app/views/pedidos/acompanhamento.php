...
function gerarTiposCandidatos(tipo, tipoRaw){
  ...
  // heurística: tratar typos de "entrega" (ex.: entrefa, entega, entrea)
  ...
  // SEMPRE coloca fallback explícito para os dois tipos principais
...
// Tenta interpretar a resposta como JSON; se não, devolve texto
...
/* ======== PARSER DE HTML (fallback mais agressivo) ======== */
function parseProdutosDeHtml(html){
  try{
    ...
    // 2) tabelas (qualquer tabela do documento)
    ...
    // 3) fallback por texto: linhas do tipo "2 x Alguma coisa"
    if (linhas.length === 0) {
...
async function tentarCarregarProdutosDeUmPedido(div) {
  ...
  try {
    ...
    // 1) com candidatos de tipo (inclui fallbacks para entrega/retirada)
    ...
    // 4) Scraping direto de páginas conhecidas
