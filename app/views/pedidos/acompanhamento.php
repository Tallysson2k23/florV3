    ...
    <div id="lista-pedidos">
        <table>
            ...
            <?php foreach ($todosPedidos as $pedido):
                ...
                $tooltipProdutos = htmlspecialchars(implode("\n", $prodLista));
            ?>
            <tr>
                <td class="col-codigo">
                ...
                <td>
                    <span class="ver-produtos" title="<?= $tooltipProdutos ?>">
                        Ver produtos
                    </span>
                </td>
...
const SOUND_FALLBACK = 'data:audio/mp3;base64,//uQZAAAAAAAAAAAAAAAAAAAA...'; // opcional
audioEl.volume = 1.0;
...
function atualizarStatus(id, tipo, selectEl) {
  const novoStatus = (selectEl && selectEl.value) ? selectEl.value : String(selectEl || '').trim();
  const anterior    = (selectEl && selectEl.getAttribute('data-status')) || '';

  idTemp = id;
  tipoTemp = tipo;
  selectTemp = selectEl;
  statusTemp = novoStatus;
  prevStatusTemp = anterior;
  ...
  if (s === 'produção' || s === 'producao') {
    ...
    modal.setAttribute('data-acao', 'status');
    return;
  }
...
function enviarStatus(id, tipo, status, extras = {}, onOk) {
  ...
  .then(() => {
    if (selectTemp) {
      selectTemp.setAttribute('data-status', status);
      aplicarClasseStatus(selectTemp, status);
    }
    if (typeof onOk === 'function') onOk();
    if (typeof atualizarTabelaAcompanhamento === 'function') atualizarTabelaAcompanhamento();
  })
  .catch(err => {
    console.error('Falha ao atualizar status:', err);
    alert('Erro ao atualizar status. Tente novamente.');
    if (selectTemp && prevStatusTemp != null) {
      selectTemp.value = prevStatusTemp;
      aplicarClasseStatus(selectTemp, prevStatusTemp);
    }
  })
  .finally(() => {
    statusTemp = idTemp = tipoTemp = selectTemp = prevStatusTemp = null;
  });
}
...
function confirmarResponsavel() {
  ...
  if (acao === 'status') {
    if (!idTemp || !tipoTemp || !statusTemp) return;
    enviarStatus(idTemp, tipoTemp, statusTemp, { responsavel });
  } else if (acao === 'impressao') {
    if (!idImpressaoTemp || !tipoImpressaoTemp) return;
    enviarStatus(
      idImpressaoTemp,
      tipoImpressaoTemp,
      'Produção',
...
function confirmarImpressao(id, tipo) {
  ...
  const modal = document.getElementById('modalResponsavel');
  modal.style.display = 'block';
  modal.setAttribute('data-acao', 'impressao');
}
...
// Habilitar som a partir do botão de login (se existir)
